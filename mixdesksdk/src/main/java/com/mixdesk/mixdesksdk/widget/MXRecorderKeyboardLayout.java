 ...
 * 描述:
 */
public class MXRecorderKeyboardLayout extends MXBaseCustomCompositeView implements MXAudioRecorderManager.Callback, View.OnTouchListener {
    /**
     * 录音的最大时间
     */
    ...
    private MXAudioRecorderManager mAudioRecorderManager;
    private float mTime;
    ...
    public MXRecorderKeyboardLayout(Context context) {
        super(context);
    }

    public MXRecorderKeyboardLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public MXRecorderKeyboardLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @Override
    protected int getLayoutId() {
        return R.layout.mx_layout_recorder_keyboard;
    }
    ...
    protected void processLogic() {
        ...
        mDistanceCancel = MXUtils.dip2px(getContext(), 10);
        mAudioRecorderManager = new MXAudioRecorderManager(getContext(), this);
    }

    private void initLevelListDrawable() {
        LevelListDrawable levelListDrawable = new LevelListDrawable();
        for (int i = 0; i < 9; i++) {
            int resId = getContext().getResources().getIdentifier("mx_voice_level" + (i + 1), "drawable", getContext().getPackageName());
            try {
                levelListDrawable.addLevel(i, i + 1, MXUtils.tintDrawable(getContext(), getResources().getDrawable(resId), R.color.mx_chat_audio_recorder_icon));
            } catch (Resources.NotFoundException e) {

            }
        }
        levelListDrawable.addLevel(9, 10, getResources().getDrawable(R.drawable.mx_voice_want_cancel));
        ...
        post(new Runnable() {
            ...
            public void run() {
                if (mCurrentState == STATE_RECORDING) {
                    ...
                    if (remainingTime <= 10) {
                        mStatusTv.setText(getContext().getString(R.string.mx_recorder_remaining_time, remainingTime));
                    }
                }
            }
        });
    ...
    private void changeState(int status) {
        if (mCurrentState != status) {
            ...
            switch (mCurrentState) {
                case STATE_NORMAL:
                    mStatusTv.setText(R.string.mx_audio_status_normal);
                    mAnimIv.setImageLevel(1);
                    break;
                case STATE_RECORDING:
                    mStatusTv.setText(R.string.mx_audio_status_recording);
                    break;
                case STATE_WANT_CANCEL:
                    mStatusTv.setText(R.string.mx_audio_status_want_cancel);
        ...
        if (mCallback != null) {
            ...
            if (!TextUtils.isEmpty(currentFilePath)) {
                ...
                if (currentFile.exists() && currentFile.length() > 6) {
                    mCallback.onAudioRecorderFinish(MXAudioPlayerManager.getDurationByFilePath(getContext(), currentFile.getAbsolutePath()), currentFile.getAbsolutePath());
                } else {
                    mAudioRecorderManager.cancel();
