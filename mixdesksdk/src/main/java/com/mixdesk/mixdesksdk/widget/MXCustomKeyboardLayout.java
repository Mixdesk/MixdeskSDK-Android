 ...
 * 描述:
 */
public class MXCustomKeyboardLayout extends MXBaseCustomCompositeView {
    ...
    private MXEmotionKeyboardLayout mEmotionKeyboardLayout;
    private MXRecorderKeyboardLayout mRecorderKeyboardLayout;
    ...
    public MXCustomKeyboardLayout(Context context) {
        super(context);
    }

    public MXCustomKeyboardLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public MXCustomKeyboardLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @Override
    protected int getLayoutId() {
        return R.layout.mx_layout_custom_keyboard;
    }
    ...
    protected void setListener() {
        mEmotionKeyboardLayout.setCallback(new MXEmotionKeyboardLayout.Callback() {
            @Override
        ...
        mRecorderKeyboardLayout.setCallback(new MXRecorderKeyboardLayout.Callback() {
            @Override
    ...
    public void changeToVoiceKeyboard() {
        MXUtils.closeKeyboard(mActivity);

        if (isCustomKeyboardVisible()) {
            showVoiceKeyboard();
        } else {
            mHandler.sendEmptyMessageDelayed(WHAT_CHANGE_TO_VOICE_KEYBOARD, MXUtils.KEYBOARD_CHANGE_DELAY);
        }
    }
    ...
    public void changeToEmotionKeyboard() {
        ...
        if (isCustomKeyboardVisible()) {
            ...
            mHandler.sendEmptyMessageDelayed(WHAT_CHANGE_TO_EMOTION_KEYBOARD, MXUtils.KEYBOARD_CHANGE_DELAY);
        }
    }
    ...
    public void changeToOriginalKeyboard() {
        closeCustomKeyboard();
        MXUtils.openKeyboard(mContentEt);
        // 打开系统键盘也延时了的，这里延时2倍再滚动到底部
        mHandler.sendEmptyMessageDelayed(WHAT_SCROLL_CONTENT_TO_BOTTOM, MXUtils.KEYBOARD_CHANGE_DELAY * 2);
    }
    ...
    private void sendScrollContentToBottomMsg() {
        mHandler.sendEmptyMessageDelayed(WHAT_SCROLL_CONTENT_TO_BOTTOM, MXUtils.KEYBOARD_CHANGE_DELAY);
    }
    ...
    public void init(Activity activity, EditText contentEt, Callback callback) {
        if (activity == null || contentEt == null || callback == null) {
            throw new RuntimeException(MXCustomKeyboardLayout.class.getSimpleName() + "的init方法的参数均不能为null");
        }
